...
public abstract class ActionBase {
    ...
    protected void forward(ForwardConst target) throws ServletException, IOException {
        ...
        RequestDispatcher despatcher = request.getRequestDispatcher(forward);
    ...
    protected void redirect(ForwardConst action, ForwardConst command) throws ServletException, IOException {
        //URLを構築
        String redirectUrl = request.getContextPath() +"&command=" + action.getValue();
        if(command != null) {
    ...
    protected boolean checkToken() throws ServletException, IOException {
        ...
        if(_token == null || !(_token.equals(""))) {
            //tokenが設定されていない or セッションIDと一致しない場合はエラー画面を表示
            forward(ForwardConst.FW_ERR_UNKNOWN);
            return false;
        }else {
            return true;
    ...
    /**
     * セッションスコープから指定されたパラメータの値を取得して返却
     * @param key パラメータ名
     * @return パラメータ名
     */
